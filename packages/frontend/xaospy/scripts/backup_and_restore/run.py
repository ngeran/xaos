...
async def main():
    ...
    parser.add_argument(
        "--hostname", help="A single hostname or a comma-separated list of hostnames."
    )
    parser.add_argument(
        "--inventory_file",
        help="Path to a YAML inventory file for targeting multiple devices.",
    )
    parser.add_argument(
        "--username", required=True, help="The username for device authentication."
    )
    parser.add_argument(
        "--password", required=True, help="The password for device authentication."
    )
    parser.add_argument(
        "--backup_path",
        default="/backups",
        help="The directory where backups are stored.",
    )
    parser.add_argument("--backup_file", help="The specific backup file to restore.")
    parser.add_argument(
        "--type",
        default="override",
        choices=["override", "merge", "update"],
        help="The restore method.",
    )
    parser.add_argument(
        "--confirmed_commit_timeout",
        type=int,
        default=0,
        help="Timeout for confirmed commit rollback.",
    )
    parser.add_argument(
        "--commit_timeout",
        type=int,
        default=300,
        help="Timeout for the commit operation itself.",
    )
    ...
    try:
            ...
            if args.inventory_file:
                ...
                if not inventory_path.is_file():
                    raise FileNotFoundError(
                        f"Inventory file not found at the specified path: {inventory_path}"
                    )
                ...
                raise ValueError(
                    "No target specified. Use --hostname or --inventory_file for backup."
                )
        ...
        # ---------------------------------------------------------------------------------------------
            ...
            if not args.hostname:
                raise ValueError(
                    "A target --hostname is required for the restore command."
                )
            if not args.backup_file:
                raise ValueError(
                    "A --backup_file name is required for the restore command."
                )
            send_progress(
                "info",
                "OPERATION_START",
