/**
 * File Path: src/plugins/AppLayout/components/Header.jsx
 * Version: 2.2.0
 * Description: Header component featuring navigation, theme toggle,
 *              and WebSocket connection status with a detailed dropdown.
 *
 * Key Features:
 * - Displays application logo with a home link.
 * - Integrates `MegaMenu` for complex navigation structures.
 * - Provides a theme toggle button that switches between Dark, Light, and Yellow modes.
 * - Uses `WebSocketStatus` component to show real-time backend connection status and log.
 * - Fully responsive design, adapting to mobile, tablet, and desktop views.
 * - Fetches navigation data from backend API instead of local JSON.
 *
 * Detail How-To Guide:
 * 1. Place this component within a `nav` element in your `AppLayout`.
 * 2. Ensure `MegaMenu` and `WebSocketStatus` components are correctly imported and rendered.
 * 3. The theme toggle automatically uses the `useLayoutContext` to manage themes.
 * 4. Pass `config`, `isMobile`, `activeMegaMenu`, `onMenuEnter`, `onMenuLeave`, and `onError`
 *    props from `AppLayout` for full functionality.
 * 5. Navigation data is now fetched from http://localhost:3001/api/navigation
 *
 * Change Log:
 * - 2.2.0 (2025-09-14): Updated to fetch navigation data from backend API instead of local JSON.
 * - 2.1.1 (2023-10-27): Updated header comments to meet new documentation standards.
 * - 2.1.0 (2023-10-20): Integrated `WebSocketStatus` component for connection monitoring.
 * - 2.0.0 (Original): Initial header with mega menu and theme toggle.
 */

// ================================================
// IMPORTS
// ================================================
import React from 'react';
import { useLayoutContext } from '../AppLayout.jsx';
import MegaMenu from './MegaMenu.jsx';
import WebSocketStatus from './WebSocketStatus.jsx';

// ================================================
// HEADER COMPONENT
// ================================================

const Header = ({
  config,
  isMobile,
  activeMegaMenu,
  onMenuEnter,
  onMenuLeave,
  onError,
  className = '',
  style = {},
}) => {
  const layoutContext = useLayoutContext();
  const { toggleTheme, theme } = layoutContext;

  // -------------------------------
  // EVENT HANDLERS
  // -------------------------------

  /**
   * Handles the click event for the theme toggle button.
   * Toggles the application's theme and logs any errors.
   */
  const handleThemeToggleClick = () => {
    try {
      toggleTheme?.();
    } catch (error) {
      console.error('Failed to toggle theme:', error);
      onError?.(error, 'theme-toggle');
    }
  };

  // -------------------------------
  // RENDER FUNCTIONS
  // -------------------------------

  /**
   * Renders the application logo with a link to the home page.
   * @returns {JSX.Element} The logo element.
   */
  const renderLogo = () => (
    <div className="nav-logo">
      <a href="/" aria-label="Effortel Home">
        <svg data-v-4b5b0323="" width="30" height="30" viewBox="0 0 19 20" fill="none" xmlns="http://www.w3.org/2000/svg" class="icon"><path d="M6.20742 9.02441H0.12651C0.0566404 9.02441 0 9.08105 0 9.15092V9.85095C0 9.92081 0.0566404 9.97745 0.12651 9.97745H6.20742C6.27728 9.97745 6.33392 9.92081 6.33392 9.85095V9.15092C6.33392 9.08105 6.27728 9.02441 6.20742 9.02441Z" fill="currentColor"></path><path d="M18.8754 9.02441H12.7945C12.7246 9.02441 12.668 9.08105 12.668 9.15092V9.85095C12.668 9.92081 12.7246 9.97745 12.7945 9.97745H18.8754C18.9453 9.97745 19.0019 9.92081 19.0019 9.85095V9.15092C19.0019 9.08105 18.9453 9.02441 18.8754 9.02441Z" fill="currentColor"></path><path d="M9.02441 0.12657V6.20747C9.02441 6.27734 9.08105 6.33398 9.15092 6.33398H9.85094C9.92081 6.33398 9.97745 6.27734 9.97745 6.20747V0.12657C9.97745 0.0567002 9.92081 5.96046e-05 9.85094 5.96046e-05H9.15092C9.08105 5.96046e-05 9.02441 0.0567002 9.02441 0.12657Z" fill="currentColor"></path><path d="M9.0332 12.8028V18.8837C9.0332 18.9536 9.08984 19.0103 9.15971 19.0103H9.85974C9.9296 19.0103 9.98624 18.9536 9.98624 18.8837V12.8028C9.98624 12.733 9.9296 12.6763 9.85974 12.6763H9.15971C9.08984 12.6763 9.0332 12.733 9.0332 12.8028Z" fill="currentColor"></path><path d="M8.0754 7.40936C8.07773 7.40703 8.07772 7.40325 8.07539 7.40093L3.17103 2.50499C3.12043 2.45439 3.04452 2.45439 2.99392 2.50499L2.50475 2.99417C2.45415 3.04477 2.45415 3.12068 2.50475 3.17128L7.90425 8.57078C7.98157 8.6481 8.07962 8.72957 8.07962 8.83892V10.1715C8.07962 10.2305 8.05431 10.2811 8.02058 10.3233L2.50475 15.8391C2.45415 15.8897 2.45415 15.9656 2.50475 16.0162L2.99392 16.5054C3.04452 16.556 3.12043 16.556 3.17103 16.5054L8.07118 11.6053L8.3495 11.3269C8.77964 10.8968 9.02422 10.3149 9.02422 9.70762V9.31122C9.02422 8.70397 8.77964 8.12203 8.3495 7.6919L8.0754 7.41779C8.07307 7.41546 8.07307 7.41169 8.0754 7.40936Z" fill="currentColor"></path><path d="M10.99 8.68324C10.99 8.68467 10.9917 8.68538 10.9927 8.68437L16.5058 3.17128C16.5564 3.12067 16.5564 3.04477 16.5058 2.99417L16.0166 2.50499C15.966 2.45439 15.8901 2.45439 15.8395 2.50499L10.9394 7.40514L10.661 7.68346C10.2309 8.1136 9.98633 8.69554 9.98633 9.30279V9.69918C9.98633 10.3064 10.2309 10.8884 10.661 11.3185L10.9394 11.5968L15.8395 16.497C15.8901 16.5476 15.966 16.5476 16.0166 16.497L16.5058 16.0078C16.5564 15.9572 16.5564 15.8813 16.5058 15.8307L11.1063 10.4312C11.029 10.3539 10.9309 10.2724 10.9309 10.1631V8.83048C10.9309 8.77311 10.9548 8.7237 10.9871 8.68226C10.9881 8.68106 10.99 8.68172 10.99 8.68324Z" fill="currentColor"></path></svg>
      </a>
    </div>
  );

  /**
   * Renders the theme toggle button.
   * Allows users to switch between dark, light, and yellow themes.
   * @returns {JSX.Element} The theme toggle button.
   */
  const renderThemeToggle = () => (
    <button
      className="icon-btn"
      id="themeToggle"
      onClick={handleThemeToggleClick}
      aria-label="Toggle theme"
      title={`Switch to ${theme === 'dark' ? 'light' : 'dark'} mode`}
    >
      <svg className="sun-icon" viewBox="0 0 24 24" fill="none">
        <circle cx="12" cy="12" r="4" stroke="currentColor" strokeWidth="2" />
        <path
          d="M12 2V4M12 20V22M22 12H20M4 12H2"
          stroke="currentColor"
          strokeWidth="2"
          strokeLinecap="round"
        />
        <path
          d="M19.07 4.93L17.66 6.34M6.34 17.66L4.93 19.07M19.07 19.07L17.66 17.66M6.34 6.34L4.93 4.93"
          stroke="currentColor"
          strokeWidth="2"
          strokeLinecap="round"
        />
      </svg>

      <svg className="moon-icon" viewBox="0 0 24 24" fill="none">
        <path
          d="M21 12.79A9 9 0 1111.21 3 7 7 0 0021 12.79z"
          stroke="currentColor"
          strokeWidth="2"
          strokeLinecap="round"
          strokeLinejoin="round"
        />
      </svg>
    </button>
  );

  /**
   * Renders the mobile menu toggle button (hamburger icon).
   * Hidden on larger screens via CSS.
   * @returns {JSX.Element} The mobile menu toggle button.
   */
  const renderMobileMenuToggle = () => (
    <button className="mobile-menu-toggle" aria-label="Toggle navigation">
      <span></span>
      <span></span>
      <span></span>
    </button>
  );

  // -------------------------------
  // MAIN RENDER
  // -------------------------------

  return (
    <div className="nav-container">
      {renderLogo()}

      <MegaMenu
        activeMenu={activeMegaMenu}
        onMenuEnter={onMenuEnter}
        onMenuLeave={onMenuLeave}
        onError={onError}
      />

      <div className="nav-actions">
        {/* WebSocketStatus component handles its own state and UI */}
        <WebSocketStatus />

        {renderThemeToggle()}
        {renderMobileMenuToggle()}
      </div>
    </div>
  );
};

// ================================================
// EXPORT
// ================================================
export default Header;
