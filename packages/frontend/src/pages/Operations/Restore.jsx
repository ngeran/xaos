 ...
 * - Tab-based interface for backup and restore: Restore, Execute, Results
 ...
 * - Tab-based navigation with step indicator for stable input handling
 *
 * Architecture:
 ...
 * - Tab-based navigation with step indicator
 *
 * Dependencies:
  ...
  useEffect(() => {
    const fetchSidebarItems = async () => {
      try {
        ...
        // Set sidebar items directly from API response
        setSidebarItems(data || []);
        setError(null);
      } catch (err) {
        console.error('Failed to fetch sidebar items:', err);
        setError(err.message);
        // Fallback to minimal sidebar items
        setSidebarItems([
          {
            id: 'backup',
            title: 'Backup Device',
            icon: 'Database',
            description: 'Create device configuration backups'
          },
          {
            id: 'restore',
            title: 'Restore Device',
            icon: 'Upload',
            description: 'Restore device configurations from backups'
          }
        ]);
      } finally {
        setLoading(false);
  ...
  const renderMainContent = () => {
    if (activeTab === 'backup') {
                    ...
                    <div className="bg-muted dark:bg-gray-700 p-4 rounded-md">
                      ...
                      <ul className="space-y-1 text-sm text-gray-700 dark:text-gray-300">
                        <li className="flex">
                          ...
                          <span>
                            {parameters.hostname || parameters.inventory_file.replace('.yaml', '').replace('.yml', '') || 'Unknown'}
                          </span>
                        </li>
                        <li className="flex">
      ...
      return (
        <div className="w-full max-w-4xl mx-auto">
          <div className="mb-6 px-6">
            <h2 className="text-2xl font-bold text-gray-900 dark:text-gray-100">Device Configuration Restore Tool</h2>
            <p className="text-muted-foreground dark:text-gray-400">Restore device configurations from backups</p>
          </div>
          <div className="mb-6 px-6">
            <div className="flex items-center space-x-4">
              {[1, 2, 3].map((step) => (
          ...
          <Tabs value={currentTab} onValueChange={setCurrentTab} className="w-full">
            <TabsList className="grid w-full grid-cols-3 bg-muted dark:bg-black rounded-lg p-1">
              <TabsTrigger
                value="restore"
              ...
              <TabsTrigger
                ...
                className="data-[state=active]:bg-white dark:data-[state=active]:bg-gray-700 data-[state=active]:text-gray-900 dark:data-[state=active]:text-gray-100 disabled:opacity-50"
              >
                Execute
              </TabsTrigger>
              <TabsTrigger
